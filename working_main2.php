<?php
    ...
    for($i=0;$i<sizeof($Date_List);$i++){
        for($p=0;$p<2;$p++){
            for($j=0;$j<sizeof($Room_List);$j++){
                if($Shift_List[$p]!= $_SESSION["search_shift1"] )continue;
                if($Date_List[$i]!= $_SESSION["search_date1"] )continue;
                if($Course_List[$k]!=$_SESSION["search_subject1"])continue;
    ...
    ;
      $op_name="".$_SESSION["search_date1"]."_".$day_n."_".$_SESSION["search_shift1"]."_".$_SESSION["search_subject1"];
    $op_name=$op_name.'.pdf';
